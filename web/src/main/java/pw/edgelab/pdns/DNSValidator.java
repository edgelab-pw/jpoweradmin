...
import pw.edgelab.pdns.dao.RecordDAO;
	...
	public boolean isValid(Record record, int domainId){
		...
		switch(record.getType()){
			case A:{
				...
				if(this.recordDAO.recordNameExists(record.getName(), DNSType.CNAME, domainId)){
			...
			case CNAME: {
				...
				if(this.recordDAO.recordContentExists(record.getName(), DNSType.NS, domainId)){
					this.statusMessages.addFromResourceBundle(Severity.ERROR, "DNSValidator.DNS_CNAME_MX_NS");
					return false;
				}
				if(this.recordDAO.recordContentExists(record.getName(), DNSType.MX, domainId)){
					this.statusMessages.addFromResourceBundle(Severity.ERROR, "DNSValidator.DNS_CNAME_MX_NS");
					return false;
				}
				if(this.recordDAO.recordNameExists(record.getName(), DNSType.A, domainId)){
					this.statusMessages.addFromResourceBundle(Severity.ERROR, "DNSValidator.DNS_CNAME_A", record.getName());				
					return false;
				}
				if(this.recordDAO.recordNameExists(record.getName(), DNSType.AAAA, domainId)){
					this.statusMessages.addFromResourceBundle(Severity.ERROR, "DNSValidator.DNS_CNAME_A", record.getName());				
					return false;
				}
				if(this.recordDAO.recordNameExists(record.getName(), DNSType.CNAME, domainId)){
			...
			case MX:{
				if(!this.isValidFQDN(record.getName())) return false;
				if(!this.isValidFQDN(record.getContent())) return false;
				if(this.recordDAO.recordContentExists(record.getContent(), DNSType.CNAME, domainId)){
					this.statusMessages.addFromResourceBundle(Severity.ERROR, "DNSValidator.DNS_CNAME_Exists", record.getName());
					return false;
				}
				return true;
