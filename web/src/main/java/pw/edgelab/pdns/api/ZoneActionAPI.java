package pw.edgelab.pdns.api;

import pw.edgelab.pdns.DNSValidator;
import pw.edgelab.pdns.dao.RecordDAO;
import pw.edgelab.pdns.dao.ZoneDAO;
...
public class ZoneActionAPI {
	...
	public void addZone(Domain domain){
		...
		if(existingDomain != null){
			...
			if(domain.getType() == Domain.DomainType.MASTER){
	...
	public void editZone(Domain domain){
		...
		Domain.DomainType newType = domain.getType();
		Domain.DomainType oldType = domainToUpdate.getType();
		...
		if(newType != oldType && newType == Domain.DomainType.MASTER){
	...
	public void updateRecord(Record record, @PathParam("zoneName") String zoneName){
		...
		this.logger.info("Loading record "+record.getId());
